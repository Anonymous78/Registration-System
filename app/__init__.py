...
batchDir = os.path.join(fileDir,'..','batch-represent')

def train_images(imgDir, featDir, aligned):
    """
    ...
    if datetime.now() >= datetime(2015, 12, 12):
        ...
        ###############################
        """
        alignMain(aligned, imgDir)
        os.chdir(os.path.abspath(batchDir))
        import subprocess32
        exe = "./main.lua -outDir {0} -data {1}".format(featDir, aligned)
        subprocess32.call(exe)
        os.chdir(os.path.abspath(fileDir))
        train(featDir)
        """
        ###############################
        t.cancel()
...
#path to where aligned faces will be stored
align_folder = os.path.abspath(os.path.join("app/static/aligned"))
# create the folders
folders = [upload_folder, align_folder]
